...
; ilk ve son  index'ler arasında ekleme yapılabilir ( 0 - list size)
; range dışında index girilirse  liste yi döner 
(defun insert-n(lst number index) 
	(if (= index 0)
		(cons number (insert-n lst number (- index 1)))
		(if (null lst)
			nil 
			(cons (car lst) (insert-n (cdr lst) number (- index 1)))
		)
	)
)
